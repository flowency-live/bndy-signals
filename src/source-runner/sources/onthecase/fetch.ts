 ...
 * On The Case Fetch
 *
 * JS-rendering fetch for onthecasemusic.co.uk/gigs.
...
/**
 * Fetch On The Case page using Puppeteer for JS rendering.
 * The site is client-rendered so we need a headless browser.
 */
export async function fetchOnTheCaseSource(
  config: SourceConfig,
  _run: SourceRun
  ...
  try {
    ...
    // Wait for gig content to appear (the site renders gig listings dynamically)
