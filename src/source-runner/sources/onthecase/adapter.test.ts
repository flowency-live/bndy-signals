...
describe('onTheCaseAdapter', () => {
  ...
  beforeEach(() => {
    ...
    // Setup Puppeteer mock - $eval returns rendered text with multi-line dates
    const mockPage = {
      goto: vi.fn().mockResolvedValue(undefined),
      waitForSelector: vi.fn().mockResolvedValue(undefined),
      $eval: vi.fn().mockResolvedValue(`Thursday 11
June 2026
...
9:00 PM / FREE`),
  ...
  it('should park skipped gigs', async () => {
    // Setup mock to return content with TBC - $eval returns text with multi-line dates
    const mockPage = {
      goto: vi.fn().mockResolvedValue(undefined),
      waitForSelector: vi.fn().mockResolvedValue(undefined),
      $eval: vi.fn().mockResolvedValue(`Thursday 11
